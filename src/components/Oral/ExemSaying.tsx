...
const ExemSaying = () => {
  ...
  const [listQuestion, setListQuestion] = useState<SentenceType[]>([])
  ...
  const onContinute = () => {
    ...
    if (exitsHistory) {
      ...
      if (listQuestion[sentencesIndex].words.replace('.', '').toLocaleLowerCase() === value.replace('.', '').toLocaleLowerCase()) {
  ...
  const getSentences = async () => {
    const { payload } = await dispatch(getListSentencesSlice())
    let listSenten: SentenceType[] = []
    listDayByWeek.map((e) => {
      listSenten.push(...payload.filter((item: SentenceType) => item?.practiceActivity?.day === e._id))
    })
    setListQuestion(listSenten)
    console.log("listSenten", listSenten);
  ...
  const onFinnish = async () => {
    ...
    if (learningProgress) {
      dispatch(editLearningProgressSlice({ ...learningProgress, oralWeekSentencesScore: score }))
    }
    ...
    const { payload: data2 } = await dispatch(addHistorySlide({
      ...
      result: score >= 8 ? 1 : 0,
      type: "Thi tuần (Câu nói thông dụng)"
    }))
    if (data2) {
      ...
      <div className="exem__vocabulary__quiz">
        ...
        {checkSubmit ?
          <div className="result__exam__info">
            ...
            <div className="btn__control__result__exem">
              ...
              <NavLink to={`/learning/oralWeekSaying/${weekId}/${dayId}`}>  <button >Làm lại</button></NavLink>
            ...
            <div className="exam__content__quiz">
              ...
              <div className='flex justify-between mt-4'>
                ...
                <div className="status__exem__quiz">
                  ...
                  <div className='flex  my-auto'><div className='my-auto'>Thời gian :</div> <span><TimeExamWeeks addTime={onHandlerTimeResult} seconds={720} stop={checkSubmit ? true : false} /></span></div>
                </div>
              </div>
            </div>
          </div>
        ...
        <div className="lst__answered__oral__vocabulary">
          <div className=' hidden'>
            <TimeExamWeeks addTime={onHandlerTimeResult} seconds={720} stop={checkStopTime ? true : false} />
          </div>
